    ...
    reducers: {
        ...
        calculateSupportsCount: (state, action) => {
            if (state.support_type === 'type1') {
                ...
                const totalSupports = (state.NO_PAYERS_PER_ROW + 1) * (state.NO_PAYERS_PER_COLUMN + 1) * state.count + state.slabs_count
                ...
                // prettier-ignore
                state.TOTAL_NO_PEDESTALS = (2 * y56) + (y58 * y57);
                state.supports_count = state.TOTAL_NO_PEDESTALS * state.count + state.slabs_count
                // ======== B25 =========
            }
        },
